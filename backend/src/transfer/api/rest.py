...
from src.transfer.api.dependencies import FromTransferClientDepend, ToTransferClientDepend, TransferClientDepend, \
    TransferUoWDepend
...
from src.transfer.application.use_cases.run_favorite_transfer import RunFavoriteTransferUseCase
from src.transfer.application.use_cases.run_playlist_transfer import RunPlaylistTransferUseCase
from src.transfer.domain.dtos import PlaylistReadDTO, PlaylistTracksListDTO, TrackReadDTO, TransferAlbumCreateDTO, \
    TransferPlaylistCreateDTO, TransferReadDTO, UserAlbumListDTO, UserPlaylistListDTO, UserSourceConnectDTO, \
    TransferFavoriteCreateDTO
...
@router.post("/favorite", response_model=TransferReadDTO)
...
async def get_user_playlists(transfer_client: TransferClientDepend, uow: TransferUoWDepend,
                             params: UserPlaylistListDTO = Depends()):
...
async def get_user_favorite_tracks(transfer_client: TransferClientDepend, uow: TransferUoWDepend,
                                   params: UserPlaylistListDTO = Depends()):
...
async def get_user_playlist_tracks(transfer_client: TransferClientDepend, uow: TransferUoWDepend,
                                   params: PlaylistTracksListDTO = Depends()):
...
async def get_user_album_tracks(transfer_client: TransferClientDepend, uow: TransferUoWDepend,
                                params: PlaylistTracksListDTO = Depends()):
    raise HTTPException(400, detail="Not implemented")


@router.get("/album", response_model=list[PlaylistReadDTO])
async def get_user_albums(transfer_client: TransferClientDepend, uow: TransferUoWDepend,
                          params: UserAlbumListDTO = Depends()):
